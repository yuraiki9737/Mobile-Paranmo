...
class CustomViewEmail: TextInputEditText, View.OnTouchListener {
    ...
    private fun init() {
        clearEmail = ContextCompat.getDrawable(context, R.drawable.ic_round_close_24) as Drawable // x button
    ...
    private fun showClearButton() {
        setButtonDrawables(endOfTheText = clearEmail)
    }
    ...
    override fun onTouch(v: View?, event: MotionEvent): Boolean {
        if (compoundDrawables[2] != null) {
            ...
            if (layoutDirection == View.LAYOUT_DIRECTION_RTL) {
                clearButtonEnd = (clearEmail.intrinsicWidth + paddingStart).toFloat()
                if (event.x < clearButtonEnd) isClearButtonClicked = true
            } else {
                clearButtonStart = (width - paddingEnd - clearEmail.intrinsicWidth).toFloat()
            ...
            if (isClearButtonClicked) {
                return when (event.action) {
                    MotionEvent.ACTION_DOWN -> {
                        clearEmail = ContextCompat.getDrawable(context, R.drawable.ic_round_close_24) as Drawable
